...
from src.services.site_routing import detect_site, group_site, sites_conflict
from src.services.ticket_classifier import TicketClassifier, get_ticket_classifier

logger = logging.getLogger(__name__)

# Predictions at or below this confidence are treated as low-confidence.
...
def extract_location_path(raw_ticket: dict[str, Any]) -> str | None:
    """
    Extract the location as parent\\child (e.g., 'HUP\\RAVDIN') from a raw
    Athena ticket, resolving the location GUID to its full hierarchical path
    via LOCATION_GUID_TO_FULLNAME.

    Shared by Feature #3 (assignment) and Feature #2 (chatbot) so both resolve
    locations identically. Returns the last two path segments, a leaf name, or
    None when no location information is present.
    """
    import re
...
class AssignmentService:
    ...
    async def recommend_assignment(
        ...
        # Step 5: Site-aware re-ranking (UPHS vs LGH guardrail)
        # Detect the ticket's organization from its location + text, then avoid
        # recommending a support group that belongs to the *other* organization.
        ticket_site = detect_site(
            ticket_info.location or "",
            ticket_info.title or "",
            ticket_info.description or "",
        )
        top_prediction, site_adjusted, site_warning = self._apply_site_guardrail(
        ...
        # Build alternatives (all predictions except the chosen top), preserving
        # ranked order so a demoted cross-site pick still appears as an option.
        alternatives = [
            ClassifierPrediction(
            ...
            if p is not top_prediction and p["confidence"] > 0.001
        ]
    ...
    @staticmethod
    def _apply_site_guardrail(
        predictions: list[dict[str, Any]],
        ticket_site: str | None,
    ) -> tuple[dict[str, Any], bool, str | None]:
        """
        Choose the top prediction while avoiding UPHS/LGH cross-site routing.

        Args:
            predictions: Ranked classifier predictions (highest confidence first),
                each a dict with 'support_group' and 'confidence'.
            ticket_site: Detected site of the ticket (SITE_UPHS / SITE_LGH / None).

        Returns:
            (chosen_prediction, site_adjusted, site_warning)

        Behavior:
        - If the ticket site is unknown, keep the classifier's original top pick.
        - If the classifier's top pick already matches (or is site-neutral for)
          the ticket site, keep it.
        - If the top pick belongs to the *other* site, promote the highest-ranked
          prediction whose group is same-site or site-neutral, and set
          site_adjusted=True.
        - If NO prediction is same-site/neutral, keep the original top pick but
          set a site_warning so the analyst verifies routing manually.
        """
        top = predictions[0]
        ...
        top_group_site = group_site(top["support_group"])
        if not sites_conflict(ticket_site, top_group_site):
            # Same site or site-neutral top pick — no adjustment needed.
            return top, False, None

        # Top pick conflicts with the ticket site. Find the best non-conflicting
        # prediction (same-site or site-neutral).
        for pred in predictions:
            pred_site = group_site(pred["support_group"])
            if not sites_conflict(ticket_site, pred_site):
                logger.info(
                    "Site guardrail: demoted cross-site pick '%s' (%s) in favor "
                    "of '%s' for a %s ticket.",
                    top["support_group"], top_group_site,
                    pred["support_group"], ticket_site,
                )
                return pred, True, None

        # Every prediction conflicts with the ticket site — keep original, warn.
        warning = (
            f"All classifier predictions appear to belong to a different site "
            f"than this {ticket_site} ticket. Verify support-group routing manually."
        )
        logger.warning("Site guardrail: %s", warning)
        return top, False, warning
    ...
    def _extract_location_path(raw_ticket: dict[str, Any]) -> str | None:
        ...
        Thin wrapper around the shared module-level ``extract_location_path`` so
        Feature #2 (chatbot) and Feature #3 (assignment) resolve locations
        identically.
        """
        return extract_location_path(raw_ticket)
